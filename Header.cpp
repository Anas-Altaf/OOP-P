...
bool isValidDate(int day, int month, int year) {
	...
	int daysInMonth = 31; // Assuming 31 days by default
...
bool validateEmail(const std::string& email) {
	...
	int dotSymbolPosition = -1;

	// Iterate through each character in the email
	for (int i = 0; i < email.length(); i++) {
		if (email[i] == '@') {
			// Check if @ symbol already found or if it's the first character
			if (atSymbolFound || i == 0) {
				return false;
			}
			atSymbolFound = true;
			atSymbolPosition = i;
			...
			// Check if . symbol already found or if it appears before @ symbol
			if (dotSymbolFound || atSymbolPosition > i) {
				return false;
			}
			dotSymbolFound = true;
			dotSymbolPosition = i;
	...
	// Check if @ and . symbols found, and if they are in valid positions
	return atSymbolFound && dotSymbolFound && dotSymbolPosition - atSymbolPosition > 1 && dotSymbolPosition < email.length() - 1;
}
...
void User::set()
{
	...
	do {
		...
		{
			validity = false;
			cout << "\nPlease Enter 6 digit Phone Number!\n";
		}
	} while (validity == false);
	do {
		validity = true;
		cout << "Enter Gender = ";
		cin >> gender;
		if (gender == "M" || gender == "m" || gender == "male" || gender == "Male" || gender == "MALE" || gender == "F" || gender == "f" || gender == "female" || gender == "Female" || gender == "FEMALE")
		{
			validity = true;
		}
		else
	...
	do {
		...
		if (ageDays < 0) {
			ageMonths--;
			ageDays += 31; // Assuming maximum 31 days in a month
		}

		if (ageMonths < 0) {
			ageYears--;
			ageMonths += 12; // Assuming 12 months in a year
		}
			...
			else if (helpingfile == "Staffs.txt")
			{
				...
				while (!fin.eof() && count < DataCalculate(helpingfile))
				{
					...
					while (att != 'P' && att != 'A')
					{
						...
						string b_nam;
						int bloc = -1;
						while (!bfin.eof() && bcount < DataCalculate("Bonus.txt"))
						{
							cout << "\nExcessing!";
							++bcount;
							bfin >> b_ID >> b_nam >> points;
							if (ID == b_ID)
							{
								if (att == 'P')
								{
							...
							else if(bcount==DataCalculate("Bonus.txt")-1)
							{
								cout << "\nNew Bonus Added";
								points = 10;
							}
